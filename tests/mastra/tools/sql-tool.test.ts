...
describe('SQL Tool', () => {
  ...
  const createMockResult = (rows: any[] = [], rowsAffected = 0, lastInsertRowid?: any) => ({
    rows,
    columns: [],
    columnTypes: [],
    rowsAffected,
  ...
  describe('Tool Configuration', () => {
    ...
    it('should have correct input schema', () => {
      ...
      expect(() => inputSchema.parse({ sql: 'SELECT * FROM users' })).not.toThrow();
  ...
  describe('Forbidden Operation Detection', () => {
    it('should forbid CREATE statements', async () => {
      ...
      expect(result.result).toContain('ERROR: Forbidden SQL operation detected: CREATE');
    ...
    it('should forbid DROP statements', async () => {
      ...
      expect(result.result).toContain('ERROR: Forbidden SQL operation detected: DROP');
      expect(mockExecute).not.toHaveBeenCalled();
    });

    it('should forbid ALTER statements', async () => {
      const toolContext = createToolContext('ALTER TABLE users ADD COLUMN email TEXT');
      const result = await sqlTool.execute(toolContext);
      
      expect(result.result).toContain('ERROR: Forbidden SQL operation detected: ALTER');
      expect(mockExecute).not.toHaveBeenCalled();
    });
    ...
    it('should forbid TRUNCATE statements', async () => {
      ...
      expect(result.result).toContain('ERROR: Forbidden SQL operation detected: TRUNCATE');
      expect(mockExecute).not.toHaveBeenCalled();
    });

    it('should detect forbidden operations in multiple statements', async () => {
      const toolContext = createToolContext('SELECT * FROM users; DROP TABLE users;');
      const result = await sqlTool.execute(toolContext);
    ...
    it('should detect forbidden operations with comments', async () => {
      const toolContext = createToolContext('/* comment */ CREATE TABLE test (id INTEGER)');
      const result = await sqlTool.execute(toolContext);
      
      expect(result.result).toContain('ERROR: Forbidden SQL operation detected');
      expect(mockExecute).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Update Operation Confirmation', () => {
    it('should return confirmation message for INSERT statements', async () => {
      const toolContext = createToolContext('INSERT INTO users (name) VALUES (\'test\')');
      const result = await sqlTool.execute(toolContext);
      
      expect(result.result).toContain('WARNING: This SQL operation modifies data');
      expect(result.result).toContain('INSERT INTO users');
      expect(mockExecute).not.toHaveBeenCalled();
    });

    it('should return confirmation message for UPDATE statements', async () => {
      const toolContext = createToolContext('UPDATE users SET name = \'updated\' WHERE id = 1');
      const result = await sqlTool.execute(toolContext);
    ...
    it('should return confirmation message for multiple update operations', async () => {
      const toolContext = createToolContext('INSERT INTO users (name) VALUES (\'test\'); UPDATE users SET name = \'updated\' WHERE id = 1;');
      const result = await sqlTool.execute(toolContext);
  ...
  describe('SQL Execution Results', () => {
    ...
    it('should handle empty result sets correctly', async () => {
      ...
      const toolContext = createToolContext('SELECT * FROM users WHERE id = 999');
      const result = await sqlTool.execute(toolContext);
    ...
    it('should handle SQL injection attack patterns properly', async () => {
      const injectionTests = [
        ...
        },
        {
          ...
          expectForbidden: false,
          expectWarning: true,
          description: "Detected as update operation because DELETE is contained in the entire string"
        }
      ];
      ...
      for (const test of injectionTests) {
        ...
        if (test.expectForbidden) {
          ...
          mockExecute.mockResolvedValueOnce(createMockResult([]));
          expect(result.result).not.toContain('ERROR: Forbidden SQL operation detected');
          expect(result.result).not.toContain('WARNING');
    ...
    it('should detect bypass attacks using comments', async () => {
      const commentTests = [
        ...
        },
        {
          ...
          expectForbidden: false,
          description: "SQL with line comment (processed as single statement since only split by semicolon)"
        },
        {
  ...
  describe('Special Cases', () => {
    ...
    it('should handle SQL statements with comments correctly', async () => {
      const toolContext = createToolContext('/* This is a comment */ CREATE TABLE test (id INTEGER)');
      const result = await sqlTool.execute(toolContext);
